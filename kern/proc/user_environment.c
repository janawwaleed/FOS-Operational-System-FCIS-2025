...
//    struct WorkingSetElement *curr = LIST_FIRST(&(e->page_WS_list));
//    while (curr != NULL)
//    {
//        struct WorkingSetElement *next = LIST_NEXT(curr);
//
//    	uint32 *ptr_page_table = NULL;
//    	struct FrameInfo* ptr_frame_info = get_frame_info(e->env_page_directory, curr->virtual_address, &ptr_page_table);
//    	int perms = pt_get_page_permissions(e->env_page_directory, curr->virtual_address);
//    	if(perms & PERM_MODIFIED){
//    		pf_update_env_page(e, curr->virtual_address, ptr_frame_info);
//    	}
//    	if(ptr_frame_info != 0){
//    		unmap_frame(e->env_page_directory, curr->virtual_address);
//    		free_frame(ptr_frame_info);
//    	}
//    	LIST_REMOVE(&(e->page_WS_list), curr);
//    	kfree(curr);
//
//        curr = next;
//    }
...
//        //skip kernel page tables
//        if (pdx >= PDX((void*)KERNEL_BASE))
//            continue;
//
//        uint32 pde = e->env_page_directory[pdx];
//        if (!(pde & PERM_PRESENT))
//            continue;
//
//        //get physical address of the page table frame
//        uint32 phys_pt = EXTRACT_ADDRESS(pde);
//
//        struct FrameInfo *pt_frame = to_frame_info(phys_pt);
//        if (pt_frame != NULL) {
//            free_frame(pt_frame);
//        }
//
//        e->env_page_directory[pdx] = 0;
//    }
//    tlbflush();
//
...
{	/*Abdelaziz & Sama [PROJECT'25]*/
	...
	void *stackAllocVA = kmalloc(KERNEL_STACK_SIZE);
	uint32 numPages = KERNEL_STACK_SIZE / PAGE_SIZE;
	void* bottomVA = stackAllocVA;

	//init state (bottom)
    pt_set_page_permissions(ptr_user_page_directory, (uint32)bottomVA, 0, PERM_PRESENT);
	stackAllocVA += PAGE_SIZE;
	////////////////////////
	...
	for(int i=0; i<numPages; i++){
		...
		allocate_frame(&f);
		stackAllocVA += PAGE_SIZE;
		map_frame(ptr_user_page_directory, f, (uint32)stackAllocVA ,PERM_WRITEABLE|PERM_USER|PERM_PRESENT);
	}
	return (void*) bottomVA;

	#else
    	panic("KERNEL HEAP is OFF! create_user_kern_stack() needs USE_KHEAP to be 1");
	#endif
...
void delete_user_kern_stack(struct Env* e) {
	...
	if (e->kstack == NULL) return;

	uint32 va_start = (uint32)e->kstack;
	uint32 va_end = va_start + KERNEL_STACK_SIZE;

	for(; va_start < va_end; va_start += PAGE_SIZE){
		uint32 *ptr_page_table = NULL;
		struct FrameInfo* ptr_frame_info = get_frame_info(e->env_page_directory, va_start, &ptr_page_table);
		if(ptr_frame_info != 0){
			unmap_frame(e->env_page_directory, va_start);
			free_frame(ptr_frame_info);
		}
	}
	kfree((void*)e->kstack);
